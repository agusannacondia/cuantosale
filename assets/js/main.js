  ...
  $(window).on('load', function() {
    ...
    $(document).ready(function() {
        ...
        $("#buscarProducto").click(function(){
          ...
          var texto = $("#productoABuscar").val();
          var urlApi = "https://api.mercadolibre.com/sites/MLA/search?q=" + texto;

          $.ajax({
              url: urlApi
          }).then(function(data) {
            $("#services").removeClass("d-none");
            var res = promedio(data);
            $("#resultado").text(res);
            console.log(res);
          });
...
function promedio(data){
  ...
  for(var i=0;i<cant;i++){
    valorAcumulado += data.results[i].price;
  }

  if(cant > 0){
    var promedio = valorAcumulado / cant;  
  }
  else{
    var promedio = 0;
  }
